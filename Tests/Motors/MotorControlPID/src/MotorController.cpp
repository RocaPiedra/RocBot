...
void MotorController::controlMotor(float target_rpm, float deltaTs){
  this->TargetRPM = target_rpm;
  float output = this->PID.CalculateOutput(this->CurrentRPM, target_rpm, deltaTs);
  // motor direction
...
void MotorController::updateRPM(float max_cpr, float deltaTms){
  ...
  this->CurrentRPM = ((float)MotorController::GetPulses()/(float)max_cpr*1000/deltaTms)*60;
  this->FilteredRPM = 0.854*FilteredRPM + 0.0728*CurrentRPM + 0.0728*previous_rpm; //review filter
  this->previous_rpm = CurrentRPM;
  this->resetPulses();
...
String MotorController::GetMotorState(){
  ...
  return "&" + String(this->motorPos) + 
          ";target:" + String(this->TargetRPM)+
          ";rpm:" + String(this->CurrentRPM)+
          ";rpm_filt:" + String(this->FilteredRPM)+
          ";pwr:" + String(this->currentDir * this->PID.pwr)+
          ";pwr_filt:" + String(this->currentDir * this->PID.pwr_filt);
